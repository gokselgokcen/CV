...
namespace SürüTakip
{
    public partial class addRemove : Form
    {
        ...
        private void LoadData()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("SELECT EarTagNumber,Name,Gender,Breed,Birthdate  FROM dbo.CATTLE", conn);
            dgv1.Columns.Clear();
        ...
        private void dgv1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.ColumnIndex ==6)
            {
                ...
                if(result == DialogResult.Yes)
                {
                    ...
                    SqlConnection conn = new SqlConnection(connectionString);
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("DELETE FROM dbo.CATTLE WHERE EarTagNumber=@no", conn);
